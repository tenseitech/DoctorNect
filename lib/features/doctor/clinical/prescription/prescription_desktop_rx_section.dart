...
/// Card-based Rx layout for desktop (used when !ResponsiveLayout.isCompact).
class PrescriptionDesktopRxSection extends StatefulWidget {
  const PrescriptionDesktopRxSection({
    super.key,
...
class _PrescriptionDesktopRxSectionState extends State<PrescriptionDesktopRxSection> {
  ...
  void _selectDrug(MedicineSearchSuggestion item) {
  ...
  void _handleRemove(int index) {
    if (index < 0 || index >= widget.medicines.length) return;
    final id = widget.medicines[index].id;
  ...
  List<Widget> _buildMedicineCards(List<MedicineEntry> named) {
    return named.asMap().entries.map((e) {
      ...
      return RxMedicineCard(
        ...
        onDelete: () => _handleRemove(listIndex),
  ...
  Widget build(BuildContext context) {
    final named = namedMedicineEntries(widget.medicines);
    final workspaceHeight = rxWorkspaceHeight(context);
