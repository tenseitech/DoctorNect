...
class _ReferSpecialistDialogState extends State<ReferSpecialistDialog> {
  ...
  Future<void> _sendReferral(DoctorListing specialist) async {
    ...
    try {
      ...
      if (!mounted) return;
      setState(() {
        final existing = _pendingReferrals.indexWhere((r) => r.doctorId == specialist.id);
        if (existing >= 0) {
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Row(
              children: [
                const Icon(Icons.person_add_alt_1_outlined, color: AppColors.doctorBlue, size: 22),
                ...
                children: [
                  for (var i = 0; i < _pendingReferrals.length; i++)
                    InputChip(
                      ...
                      onDeleted: _pendingReferrals[i].sent ? null : () => _removePending(i),
            ...
            if (unsentCount > 0) ...[
              ...
              SizedBox(
                ...
                child: ElevatedButton.icon(
                  ...
                  label: Text(_saving ? 'Saving…' : 'Save $unsentCount referral(s)'),
