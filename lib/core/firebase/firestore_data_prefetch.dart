      ...
      case UserType.doctor:
        await Future.wait([
          ClinicalPrescriptionStore.instance
              ...
              .catchError((e, st) {
            if (kDebugMode) debugPrint('Doctor prescription prefetch failed: $e\n$st');
          }),
          FirestoreService.instance.pharmacyFirestore
              .fetchActiveConnectionsForDoctor(profileId, preferCache: true)
              .then((r) => PharmacyConnectionStore.instance.mergeFirestoreConnections(r.items))
              .catchError((e, st) {
            if (kDebugMode) debugPrint('Doctor pharmacy prefetch failed: $e\n$st');
          }),
          FirestoreService.instance.labConnection
              .fetchActiveConnectionsForDoctor(profileId, preferCache: true)
              .then((r) => LabConnectionStore.instance.mergeFirestoreConnections(r.items))
              .catchError((e, st) {
            if (kDebugMode) debugPrint('Doctor lab connections prefetch failed: $e\n$st');
          }),
          SharedAppointmentsStore.instance
              .refreshForDoctor(profileId, preferCache: true)
              .catchError((e, st) {
            if (kDebugMode) debugPrint('Doctor appointments prefetch failed: $e\n$st');
          }),
          DoctorProfileStore.instance
              .loadFromFirestore(profileId)
              .catchError((e, st) {
            if (kDebugMode) debugPrint('Doctor profile prefetch failed: $e\n$st');
          }),
          LabOrderStore.instance
              .refreshForDoctor(preferCache: true)
              .catchError((e, st) {
            if (kDebugMode) debugPrint('Doctor lab orders prefetch failed: $e\n$st');
          }),
          PharmacyPrescriptionStore.instance
              .refreshForDoctor(profileId, preferCache: true)
              .catchError((e, st) {
            if (kDebugMode) debugPrint('Doctor pharmacy deliveries prefetch failed: $e\n$st');
          }),
          MedicalDirectoryStore.instance
              .refreshForDoctor(preferCache: true)
              .catchError((e, st) {
            if (kDebugMode) debugPrint('Doctor medical directory prefetch failed: $e\n$st');
          }),
        ]);
      case UserType.patient:
        await Future.wait([
          FirestoreService.instance.prescription
              ...
              .then((r) => ClinicalPrescriptionStore.instance.mergeFirestoreRecords(r.items))
              .catchError((e, st) {
            if (kDebugMode) debugPrint('Patient prescription prefetch failed: $e\n$st');
          }),
          PatientProfileMock.loadFromFirestore(profileId).catchError((e, st) {
            if (kDebugMode) debugPrint('Patient profile prefetch failed: $e\n$st');
          }),
          FirestoreService.instance.appointment
              .fetchForPatient(profileId, preferCache: false)
              .then((r) => SharedAppointmentsStore.instance.mergeFromFirestore(r))
              .catchError((e, st) {
            if (kDebugMode) debugPrint('Patient appointments prefetch failed: $e\n$st');
          }),
          FirestoreService.instance.patientProfile
              .fetchHealthRecords(profileId)
              .then((r) => HealthRecordsMock.applyFromFirestore(r))
              .catchError((e, st) {
            if (kDebugMode) debugPrint('Patient health records prefetch failed: $e\n$st');
          }),
          PharmacyPrescriptionStore.instance
              .refreshForPatient(profileId, preferCache: false)
              .catchError((e, st) {
            if (kDebugMode) debugPrint('Patient pharmacy deliveries prefetch failed: $e\n$st');
          }),
          LabOrderStore.instance
              .refreshForPatient(profileId, preferCache: false)
              .catchError((e, st) {
            if (kDebugMode) debugPrint('Patient lab orders prefetch failed: $e\n$st');
          }),
          PatientLabBookingStore.instance
              .refreshForPatient(profileId, preferCache: false)
              .catchError((e, st) {
            if (kDebugMode) debugPrint('Patient blood test bookings prefetch failed: $e\n$st');
          }),
        ]);
      case UserType.medicalStore:
        await Future.wait([
          FirestoreService.instance.pharmacyFirestore
              ...
              .then((r) => PharmacyConnectionStore.instance.mergeFirestoreConnections(r.items))
              .catchError((e, st) {
            if (kDebugMode) debugPrint('Store pharmacy connections prefetch failed: $e\n$st');
          }),
          FirestoreService.instance.pharmacyFirestore
              .fetchDeliveriesForStore(profileId)
              .then((r) => PharmacyPrescriptionStore.instance.mergeFromFirestore(r))
              .catchError((e, st) {
            if (kDebugMode) debugPrint('Store deliveries prefetch failed: $e\n$st');
          }),
        ]);
      case UserType.lab:
        await Future.wait([
          FirestoreService.instance.labConnection
              ...
              .then((r) => LabConnectionStore.instance.mergeFirestoreConnections(r.items))
              .catchError((e, st) {
            if (kDebugMode) debugPrint('Lab connections prefetch failed: $e\n$st');
          }),
          ...
          FirestoreService.instance.labBooking
              ...
              .then((bookings) => LabWorklistStore.instance.mergeBookings(bookings))
              .catchError((e, st) {
